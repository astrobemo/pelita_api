...
describe("Prisma Client aggregate ", () => {
    it("Test groupby", async () => {
        ...
        for (const [key, value] of Object.entries(prismaClient)) {
            console.log(`${key}: ${value}`);
            const aggeratedCustomer = await prismaClient[key].rekam_faktur_pajak_detail.groupBy({
                by: ['customer_id'],
                _max: {
                    tanggal:true
                },
            ...
            const customers = await prismaClient[key].customer.findMany({
                where: {
                    id: {
                        in: customerIds
                    }
